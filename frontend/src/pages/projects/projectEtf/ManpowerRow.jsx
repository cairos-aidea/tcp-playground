...
export const ManpowerRow = ({
  ...
  }, [filteredStaffs, departments, projectStudioId, selectedProject, person.id, allPersons]);
  ...
  return (
    <tr className={rowBg}>
      <td
        colSpan={1}
        className={`sticky left-0 ${rowBg} z-20`}
      >
            ...
            {person.has_actual === 'yes' ? (
              <span className="flex items-center justify-between w-full text-xs font-semibold text-gray-700 px-2">
                <span className="flex items-center gap-2">
                  ...
                  <span className="font-normal text-gray-700 text-xs truncate" style={{ maxWidth: '200px' }}>
                    {person.name}
                  </span>
                </span>
      ...
      {(() => {
        ...
        Object.entries(allocations).forEach(([cellKey, value]) => {
          ...
          if (type !== 'manpower' || String(person.id) !== String(personId)) return;
          // Find the stage
          ...
          const weekObj = weeks.find(w => `${w.year}_${w.month}_${w.weekNum}` === weekKey);
          if (!weekObj) return; // Only show in visible weeks
          ...
          // Render normal overlapping stages cells
            ...
            <div className="flex h-full">
              ...
              return (
                ...
                key={cellKey}
                type="text"
                className={`border-0 border-b border-gray-200 focus:ring-1 focus:ring-blue-500 text-right px-1 h-full w-full outline-none transition-colors ${isSelected
                ...
                onPaste={(e) => {
                  ...
                  const intPart = parts[0].replace(/^0+/, '').slice(0, 3);
      ...
      {/* <td
        className={`px-2 border-r border-b border-gray-200 sticky ${rowBg} text-right text-xs`}
